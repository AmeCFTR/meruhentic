...
public class BulletTutorial : MonoBehaviour
{
    ...
    public static float BarragePrevent = 0.4f; //弾丸を撃てる感覚
    ...
    void FixedUpdate()
    {
        ...
        if (Input.GetKeyDown(KeyCode.B) && timer >= 1f && TutorialController.PermitBullet == true && AllowShoot == true)
        {
            ...
            Invoke("Allow", BarragePrevent); //一度撃つと0.4秒間撃てない
    ...
    {
        if (collision.gameObject.tag == ("Tar1"))
        {
            Destroy(this.gameObject);
        }

        if (collision.gameObject.tag == ("Tar2"))
        {
            Destroy(this.gameObject);
        }

        if (collision.gameObject.tag == ("Tar3"))
        {
            Destroy(this.gameObject);
        }
